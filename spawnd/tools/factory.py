...
def create_worker_tools(run_id: str, agent_name: str, *, parent: str='', tree_path: str=''):
    ...
    for spec in WORKER_TOOL_SPECS:
        ...
        async def handler(args: dict, _spec: CoordToolSpec=spec) -> dict:
            ...
            text = await invoke_worker_tool(_spec, run_id, agent_name, kwargs, parent=parent, tree_path=tree_path)
            return _wrap(text)
        _ = tools.append(tool(spec.name, spec.description, spec.claude_schema)(handler))
    return tools
