...
try:
    ...
    raise ImportError("spawnd.runtime.executors.openai requires the openai-agents SDK. Install with: pip install 'spawnd.dev[openai]'") from err
    ...
    async def run(self, config: AgentConfig, toolset: Toolset) -> dict:
        ...
        try:
            ...
            if is_manager:
                ...
                coord_tools = build_worker_coord_tools(config.run_id, config.name, parent=config.parent or '', tree_path=config.tree_path())
            ...
            prompt = f'{starter}\n\nTask: {config.prompt}'
            _ = logger.info(f'Starting {role_label.lower()} {config.name} on OpenAI ({model})')
            ...
            if result.final_output:
                with open(log_path, 'a') as f:
                    _ = f.write(str(result.final_output) + '\n')
            ...
            if final_status == 'completed':
                _ = logger.info(f'{role_label} {config.name} completed (estimated cost: ${total_cost:.4f})')
            ...
            _ = update_agent_status(db, config.run_id, config.name, 'timeout', 'Max iterations without completion')
