...
def evaluate_stuck_run(
    ...
    if idle_iterations >= threshold:
        logger.warning(f"Run appears stuck: {idle_iterations} iterations with no events")
        insert_event(
            db,
            run_id,
            "_system",
            "error",
            {"error": "stuck_detected", "idle_iterations": idle_iterations},
        )
