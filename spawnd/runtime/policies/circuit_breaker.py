...
def apply_circuit_breaker_policy(
    ...
    logger.warning(f"Circuit breaker tripped: {failure_count} failures >= {threshold}")
    insert_event(
        db,
        run_id,
        "_system",
    ...
    if action == "cancel_all":
        ...
        cancel_all("cancelled", "Circuit breaker tripped", include_pending=True)
        return True
    if action == "pause":
        update_plan_status(db, run_id, "paused")
        cancel_all("paused")
        return True

    logger.warning(f"Circuit breaker notification: {failure_count} failures")
    return False
