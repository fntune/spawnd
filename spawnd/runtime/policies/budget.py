...
def apply_run_budget_policy(
    ...
    logger.warning(f"Run {run_id}: cost budget exceeded (${total_cost:.2f} > ${budget:.2f}), action={action}")
    insert_event(
        db,
        run_id,
        "_system",
    ...
    if action == "cancel":
        ...
        cancel_all("cancelled", "Cost budget exceeded")
        return True
