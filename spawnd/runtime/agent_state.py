...
ALLOWED_TRANSITIONS: dict[str, set[str]] = {
    "pending": {"running", "cancelled", "failed", "paused"},
    "running": {"blocked", "checking", "completed", "failed", "timeout", "cancelled", "cost_exceeded", "paused"},
    "blocked": {"running", "timeout", "cancelled", "failed"},
    "checking": {"completed", "failed", "running", "cancelled", "timeout"},
    "paused": {"pending", "cancelled", "running"},
    "completed": set(),
    "failed": {"pending"},
    "timeout": {"pending"},
    "cancelled": set(),
...
def transition_agent_status(
    ...
    *,
    ...
    if not force and not can_transition(current_status, target_status):
        return False
    update_agent_status(db, run_id, agent_name, target_status, error)
    return True
