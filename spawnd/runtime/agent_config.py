...
def hydrate_agent_runtime_config(agent_row: sqlite3.Row, prompt: str) -> HydratedAgentRuntimeConfig:
    ...
    return HydratedAgentRuntimeConfig(
        prompt=prompt,
        check_command=agent_row["check_command"] if agent_row["check_command"] is not None else "true",
        model=agent_row["model"] if agent_row["model"] is not None else "sonnet",
        max_iterations=agent_row["max_iterations"] if agent_row["max_iterations"] is not None else 30,
        max_cost_usd=agent_row["max_cost_usd"] if agent_row["max_cost_usd"] is not None else 5.0,
        runtime=runtime,
