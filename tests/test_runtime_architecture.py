...
def test_resolve_agent_plan_config_uses_agent_overrides():
    ...
    agent = AgentSpec(name='worker', prompt='Implement task', check='true', model='haiku', max_iterations=7, max_cost_usd=1.5, on_failure='retry', retry_count=1, runtime='openai')
...
def test_resolve_agent_plan_config_prefers_role_defaults_over_plan_defaults():
    defaults = Defaults(check='pytest', model='sonnet', runtime='claude', retry_count=3)
    agent = AgentSpec(name='worker', prompt='Implement task', use_role='tester')
    resolved = resolve_agent_plan_config(agent, defaults)
...
def test_cancellation_registry_is_run_scoped():
    ...
    task_a = DummyTask()
    task_b = DummyTask()
    registry.register("run-a", "agent-1", task_a)  # pyright: ignore[reportArgumentType]
    registry.register("run-b", "agent-1", task_b)  # pyright: ignore[reportArgumentType]
    assert registry.cancel('run-a', 'agent-1') is True
    assert task_a.cancelled is True
    assert task_b.cancelled is False
    _ = registry.clear_run('run-b')
