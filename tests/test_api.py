...
def test_agent_builder_passes_through_fields():
    spec = agent('svc', 'build service', depends_on=['db'], check='pytest', model='opus', type='manager', use_role='architect', max_iterations=10, max_cost_usd=2.5, on_failure='retry', retry_count=2, env={'FOO': 'bar'})
...
async def test_run_single_agent_completes(spawnd_env):
    result = await run([agent('solo', 'do the thing', check='true')], name='api-single', use_mock=True)
    assert result.success is True
    assert result.completed == ['solo']
    assert result.failed == []
    with get_db(result.run_id) as db:
        plan = require_row(get_plan(db, result.run_id))
...
async def test_run_with_explicit_run_id(spawnd_env):
    result = await run([agent('a', 'task', check='true')], run_id='explicit-run-id-001', use_mock=True)
...
async def test_api_accepts_codex_runtime_on_agentspec(spawnd_env):
    result = await run([agent('codex', 'task', check='true', runtime='codex')], name='codex-runtime', use_mock=True)
    assert result.success is True
    with get_db(result.run_id) as db:
...
async def test_run_with_dependency_runs_both(spawnd_env):
    result = await run([agent('first', 'step 1', check='true'), agent('second', 'step 2', check='true', depends_on=['first'])], name='api-deps', use_mock=True)
    assert result.success is True
...
async def test_plan_spec_input_is_accepted(spawnd_env):
    ...
    plan = PlanSpec(name='direct-plan', agents=[AgentSpec(name='x', prompt='task', check='true')])
...
async def test_invalid_plan_raises_value_error(spawnd_env):
    ...
    with pytest.raises(ValueError, match='Invalid plan'):
        _ = await run(
            [agent("a", "x", depends_on=["b"]), agent("b", "y", depends_on=["a"])],
            use_mock=True,
        )
...
async def test_run_resume_requires_run_id(spawnd_env):
    with pytest.raises(ValueError, match='resume=True requires run_id'):
        _ = await run([agent("a", "x")], resume=True, use_mock=True)

@pytest.mark.asyncio
async def test_run_resume_rejects_stale_run_db(spawnd_env):
    stale = spawnd_env / '.spawnd' / 'runs' / 'stale-run'
    _ = stale.mkdir(parents=True)
    _ = (stale / 'spawnd.db').write_text('')
    with pytest.raises(ValueError, match='Run not found: stale-run'):
        _ = await run([agent("a", "x")], run_id="stale-run", resume=True, use_mock=True)

@pytest.mark.asyncio
async def test_unknown_dependency_raises(spawnd_env):
    with pytest.raises(ValueError, match='Invalid plan'):
        _ = await run([agent("a", "x", depends_on=["nonexistent"])], use_mock=True)
...
async def test_handoff_runs_both(spawnd_env):
    result = await handoff(agent('a', 'first', check='true'), agent('b', 'second', check='true'), use_mock=True)
    assert result.success is True
