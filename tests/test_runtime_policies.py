...
def _seed_run(run_id: str):
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'policy-tests', 'name: policy-tests')
    return db
...
def test_transition_agent_status_respects_force_flag(tmp_path, monkeypatch):
    ...
    db = _seed_run(run_id)
    _ = insert_agent(db, run_id, 'worker', 'do work')
    _ = db.execute('UPDATE agents SET status = ? WHERE run_id = ? AND name = ?', ('cancelled', run_id, 'worker'))
    _ = db.commit()
    assert transition_agent_status(db, run_id, 'worker', 'completed', current_status='cancelled', force=False) is False
    assert require_row(get_agent(db, run_id, 'worker'))['status'] == 'cancelled'
    assert transition_agent_status(db, run_id, 'worker', 'completed', current_status='cancelled', force=True) is True
    assert require_row(get_agent(db, run_id, 'worker'))['status'] == 'completed'
    _ = db.close()
...
def test_apply_run_budget_policy_warn_does_not_stop(tmp_path, monkeypatch):
    ...
    def cancel_all(status: str, message: str='') -> None:
        _ = calls.append((status, message))
    should_stop = apply_run_budget_policy(db, run_id, total_cost=12.5, budget=10.0, action='warn', cancel_all=cancel_all)
    assert should_stop is False
    assert calls == []
    assert require_row(get_plan(db, run_id))['status'] == 'running'
    _ = db.close()
...
def test_apply_circuit_breaker_policy_cancel_all_stops(tmp_path, monkeypatch):
    ...
    def cancel_all(status: str, message: str='', include_pending: bool=False) -> None:
        _ = calls.append((status, message, include_pending))
    should_stop = apply_circuit_breaker_policy(db, run_id, failure_count=3, threshold=3, action='cancel_all', cancel_all=cancel_all)
    assert should_stop is True
    assert calls == [('cancelled', 'Circuit breaker tripped', True)]
    assert require_row(get_plan(db, run_id))['status'] == 'failed'
    _ = db.close()
...
def test_apply_failure_policy_retry_resets_agent(tmp_path, monkeypatch):
    ...
    db = _seed_run(run_id)
    _ = insert_agent(db, run_id, 'worker', 'do work', on_failure='retry', retry_count=2)
    _ = db.execute('UPDATE agents SET status = ?, retry_attempt = ? WHERE run_id = ? AND name = ?', ('failed', 0, run_id, 'worker'))
    _ = db.commit()
    cancel_calls: list[str] = []

    def cancel_all(status: str, message: str='', include_pending: bool=False) -> None:
        _ = cancel_calls.append(status)
