...
from spawnd.gitops.conflict_strategy import ConflictContext, FailOnConflict, ManualConflict, SpawnResolverConflict, strategy_for_mode
...
def test_fail_on_conflict_aborts_merge_and_reports_failure():
    ...
    calls: list[str] = []
    should_continue, failure = strategy.handle(ctx, try_resolve=lambda: True, abort_merge=lambda: calls.append('abort'))
...
def test_spawn_resolver_conflict_failure_aborts_and_reports():
    ...
    calls: list[str] = []
    should_continue, failure = strategy.handle(ctx, try_resolve=lambda: False, abort_merge=lambda: calls.append('abort'))
    assert should_continue is True
    assert calls == ['abort']
