...
import asyncio
import pytest
from spawnd.storage.db import get_agent, get_response, init_db, insert_agent, insert_event, insert_plan, update_agent_status
from spawnd.tools.worker import mark_complete, report_progress
from spawnd.tools.manager import mark_plan_complete, respond_to_clarification, spawn_worker
...
def test_mark_complete_check_passes(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'worker1', 'Test task', check_command='true')
    worktree = temp_spawnd_dir / '.spawnd' / 'runs' / run_id / 'worktrees' / 'worker1'
    _ = worktree.mkdir(parents=True)
    _ = db.execute('UPDATE agents SET worktree = ? WHERE run_id = ? AND name = ?', (str(worktree), run_id, 'worker1'))
    _ = db.commit()
    _ = db.close()
    result = asyncio.run(mark_complete(run_id, 'worker1', 'Task completed'))
    assert 'completed successfully' in result
    db = init_db(run_id)
    agent = require_row(get_agent(db, run_id, 'worker1'))
    _ = db.close()
...
def test_mark_complete_check_fails(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'worker1', 'Test task', check_command='false')
    worktree = temp_spawnd_dir / '.spawnd' / 'runs' / run_id / 'worktrees' / 'worker1'
    _ = worktree.mkdir(parents=True)
    _ = db.execute('UPDATE agents SET worktree = ? WHERE run_id = ? AND name = ?', (str(worktree), run_id, 'worker1'))
    _ = db.commit()
...
def test_report_progress(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'worker1', 'Test task')
    _ = db.close()
...
def test_spawn_worker_success(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage workers', agent_type='manager')
    _ = db.close()
    ...
    db = init_db(run_id)
    agent = require_row(get_agent(db, run_id, 'manager.task1'))
    _ = db.close()
...
def test_spawn_worker_already_exists(temp_spawnd_dir):
    ...
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage workers', agent_type='manager')
    _ = insert_agent(db, run_id, 'manager.task1', 'Existing', parent='manager')
    _ = db.close()
...
def test_cancel_worker_cannot_cancel_unowned_agent(temp_spawnd_dir):
    ...
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage workers', agent_type='manager')
    _ = insert_agent(db, run_id, 'victim', 'Running task')
    _ = update_agent_status(db, run_id, 'victim', 'running')
    _ = db.close()
    result = asyncio.run(cancel_worker(run_id, 'manager', 'victim'))
    assert 'Worker not found' in result
    db = init_db(run_id)
    agent = require_row(get_agent(db, run_id, 'victim'))
    _ = db.close()
...
def test_get_worker_status_cannot_read_unowned_agent(temp_spawnd_dir):
    ...
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage workers', agent_type='manager')
    _ = insert_agent(db, run_id, 'victim', 'Task')
    _ = db.close()
...
def test_respond_to_clarification(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage workers')
    _ = insert_agent(db, run_id, 'manager.task1', 'Worker task', parent='manager')
...
def test_respond_to_clarification_rejects_unowned_request(temp_spawnd_dir):
    ...
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage workers')
    _ = insert_agent(db, run_id, 'other.task1', 'Worker task', parent='other')
...
def test_mark_plan_complete_workers_pending(temp_spawnd_dir):
    ...
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage workers', agent_type='manager')
    _ = insert_agent(db, run_id, 'manager.task1', 'Running task', parent='manager')
    _ = update_agent_status(db, run_id, 'manager.task1', 'running')
    _ = db.close()
...
def test_mark_plan_complete_success(temp_spawnd_dir):
    ...
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage workers', agent_type='manager')
    _ = insert_agent(db, run_id, 'manager.task1', 'Task 1', parent='manager')
    _ = update_agent_status(db, run_id, 'manager.task1', 'completed')
    _ = db.close()
    result = asyncio.run(mark_plan_complete(run_id, 'manager', 'All done'))
    assert 'Plan complete' in result
    db = init_db(run_id)
    agent = require_row(get_agent(db, run_id, 'manager'))
    _ = db.close()
    assert agent['status'] == 'completed'
...
def test_cancel_worker_cancels_live_task(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage', agent_type='manager')
    _ = insert_agent(db, run_id, 'manager.victim', 'work', parent='manager')
    _ = update_agent_status(db, run_id, 'manager.victim', 'running')
    _ = db.close()
    ...
    async def _run():
        ...
        _ = task_registry.register(run_id, 'manager.victim', task)
        try:
            result = await cancel_worker(run_id, 'manager', 'victim')
            await asyncio.sleep(0)
            return (result, task)
        finally:
            _ = task_registry.unregister(run_id, 'manager.victim')
    result, task = asyncio.run(_run())
    assert 'Cancelled worker' in result
    assert task.cancelled() or task.done()
    db = init_db(run_id)
    agent = require_row(get_agent(db, run_id, 'manager.victim'))
    _ = db.close()
...
def test_cancel_worker_preserves_other_terminal_states(temp_spawnd_dir):
    ...
    _ = insert_agent(db, run_id, 'manager', 'Manage', agent_type='manager')
    _ = insert_agent(db, run_id, 'manager.victim', 'work', parent='manager')
    _ = update_agent_status(db, run_id, 'manager.victim', 'cost_exceeded', 'budget')
    _ = db.close()
    result = asyncio.run(cancel_worker(run_id, 'manager', 'victim'))
...
def test_mark_complete_refuses_cancelled_agent(temp_spawnd_dir):
    ...
    _ = db.execute('UPDATE agents SET worktree = ? WHERE run_id = ? AND name = ?', (str(worktree), run_id, 'worker1'))
    _ = db.commit()
    _ = update_agent_status(db, run_id, 'worker1', 'cancelled', 'cancelled by manager')
    _ = db.close()
...
def test_mark_plan_complete_accepts_cost_exceeded_workers(temp_spawnd_dir):
    ...
    _ = insert_agent(db, run_id, 'manager', 'Manage workers', agent_type='manager')
    _ = insert_agent(db, run_id, 'manager.task1', 'Task 1', parent='manager')
    _ = update_agent_status(db, run_id, 'manager.task1', 'cost_exceeded', 'budget')
    _ = db.close()
    result = asyncio.run(mark_plan_complete(run_id, 'manager', 'All done'))
    text = result
...
def test_spawn_worker_enforces_max_subagents(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage', agent_type='manager', max_subagents=2)
    _ = insert_agent(db, run_id, 'manager.w1', 'first', parent='manager')
    _ = insert_agent(db, run_id, 'manager.w2', 'second', parent='manager')
    _ = db.close()
...
def test_spawn_worker_allows_under_max_subagents(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage', agent_type='manager', max_subagents=3)
    _ = insert_agent(db, run_id, 'manager.w1', 'first', parent='manager')
    _ = db.close()
...
def test_spawn_worker_inherits_manager_runtime_and_cost_source(temp_spawnd_dir):
    ...
    db = init_db(run_id)
    _ = insert_plan(db, run_id, 'test', 'name: test', 25.0)
    _ = insert_agent(db, run_id, 'manager', 'Manage', agent_type='manager', runtime='openai', cost_source='estimated')
    _ = db.close()
    result = asyncio.run(spawn_worker(run_id, 'manager', 'child', 'Do work'))
    assert 'Spawned worker: manager.child' in result
    db = init_db(run_id)
    worker = require_row(get_agent(db, run_id, 'manager.child'))
    _ = db.close()
